...
"""
Score Question Planner output with a cheap heuristic metric.
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument(
        "--topic-lexicon-json",
        default="",
        help='Optional JSON object to inject as `topicLexicon` into planner context (e.g. \'{"budget":["budget","cost"]}\').',
    )
    ap.add_argument("--stdin-json", action="store_true", help="Read newline-delimited JSON objects from stdin.")
    args = ap.parse_args(argv)

    topic_lex = None
    if args.topic_lexicon_json.strip():
        topic_lex = _try_json(args.topic_lexicon_json.strip())
        if not isinstance(topic_lex, dict):
            raise SystemExit("--topic-lexicon-json must be a JSON object")
    ...
    if args.stdin_json:
        for line in sys.stdin:
            ...
            if topic_lex and isinstance(ctx, str):
                ctx_obj = _try_json(ctx)
                if isinstance(ctx_obj, dict) and "topicLexicon" not in ctx_obj and "topic_lexicon" not in ctx_obj:
                    ctx_obj["topicLexicon"] = topic_lex
                    ctx = _as_json_str(ctx_obj)
    ...
    if isinstance(ctx_obj, (dict, list)):
        if topic_lex and isinstance(ctx_obj, dict) and "topicLexicon" not in ctx_obj and "topic_lexicon" not in ctx_obj:
            ctx_obj["topicLexicon"] = topic_lex
