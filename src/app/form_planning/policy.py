...
def default_psychology_plan(*, goal_intent: str, use_case: str) -> Dict[str, Any]:
    """
    Compact "form psychology" plan for the UI + DSPy context.
...
def policy_for_phase(policy: Dict[str, Any], phase: str) -> Dict[str, Any]:
    allowed = policy.get("allowedMiniTypes") if isinstance(policy, dict) else None
    max_steps = policy.get("maxStepsPerCall") if isinstance(policy, dict) else None
    ...
    return {
        ...
        "maxCalls": policy.get("maxCalls") if isinstance(policy, dict) else None,
    }


def policy_for_batch(policy: Dict[str, Any], batch_number: int) -> Dict[str, Any]:
    """
    Resolve the active phase using 1-based batch_number.
    Falls back to ContextCore/PersonalGuide for older policy shapes.
    """
    phases = policy.get("phases") if isinstance(policy, dict) else None
    if isinstance(phases, list) and phases:
        ...
        c = policy_for_phase(policy, phase_id)
...
def normalize_policy(policy: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
...
def parse_batch_policy_json(text: Any) -> Optional[Dict[str, Any]]:
    obj = _best_effort_json(text)
    if not isinstance(obj, dict):
        return None
    return normalize_policy(obj)
