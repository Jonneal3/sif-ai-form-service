...
class BatchNextStepsJSONL(dspy.Signature):
    ...
    the specific service context (industry/service + grounding_summary + service_anchor_terms).
    ...
    - Use `service_anchor_terms` and `grounding_summary` to keep options service-relevant; avoid invented facts.
    ...
    - `service_anchor_terms` / `grounding_summary`: use these as anchors; avoid invented vertical facts.
    ...
    mini_steps_jsonl: str = dspy.OutputField(
        desc=(
            ...
            "and choose concrete, service-relevant options grounded in `service_anchor_terms`/`grounding_summary`."
        )
    )
