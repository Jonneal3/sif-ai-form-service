...
from ..models import MinimalFormRequest
from ..supabase_client import build_planner_payload_from_supabase
from ..utils import normalize_step_id, sse, sse_padding
from ...pipeline.form_pipeline import next_steps_jsonl, stream_next_steps_jsonl
...
def _repo_root() -> Path:
    # api/routes/form.py -> api/routes -> api -> repo root
...
def schema_version() -> str:
    ...
    if v:
        return v
    p = _contract_dir() / "schema" / "schema_version.txt"
    try:
...
def ui_step_schema_json() -> Dict[str, Any] | None:
    p = _contract_dir() / "schema" / "ui_step.schema.json"
    try:
...
def _minimize_success_response(result: Any) -> Any:
    """
    For successful responses, return only the frontend-required sections.
    ...
    out: Dict[str, Any] = {"miniSteps": mini_steps}
