...
async def build_planner_payload_from_supabase(
    ...
    try:
        from sif_ai_form_service.form_planning.policy import normalize_policy, policy_for_batch
    ...
    if not items_list and form_plan_list:
        ...
        if normalized_policy and isinstance(batch_number, int) and batch_number > 0:
            try:
                from sif_ai_form_service.form_planning.policy import policy_for_batch
    ...
    if allowed_step_types_override:
        ...
        if normalized_policy:
            try:
                from sif_ai_form_service.form_planning.policy import policy_for_batch, policy_for_phase
...
"""
Note: this service does not ingest telemetry/feedback events.

Telemetry/feedback should be handled client-side (or by a separate analytics pipeline).
"""
